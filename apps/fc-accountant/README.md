...
Plain Japanese HTML with server-side sessions, individual passwords and a random URL segment. Public gateway host is www.froystein.jp; only /accountant is routed to this app. No Google or ChatGPT login required. Passwords are scrypt hashes in the manually managed `fc-accountant/portal-auth` Secret; never commit that Secret or its link. No documents, manifests with financial data, or credentials belong in Git or the image.
...
Provision the `portal-auth` Secret out of band with key `auth.json`: `{origin: "https://www.froystein.jp", link: <32 random bytes in base64url>, users: [{username, salt, hash, disabled?}]}`. Each password hash is 64-byte scrypt (N=16384, r=8, p=1), salt a fresh random string. Use a private temporary file, create the Secret from file, then remove that temporary hash file. Password delivery is separate from the link and should be person-specific.
