...
The dashboard is available only through the internal Gateway. After the first
successful reconciliation:

1. Open `https://analytics.talos.froystein.jp` from the internal network.
2. Sign in to Umami with its one-time upstream default credentials.
3. Replace the default password with a generated password stored in the
   password manager, then sign out and prove the old password fails.
4. Create the `www.froystein.jp` website and record its website ID for the site
   repository.

Do not make the public collector route available until step 3 is complete.
...
- the dashboard returns the Umami login page only from the internal network;
